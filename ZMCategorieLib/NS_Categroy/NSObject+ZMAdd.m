...
+ (void)zm_setTargetModel:(NSObject *)targetModel fromModel:(NSObject *)model {
    NSArray *properties = [model zm_propertyList];
    for (NSString *key in properties) {                 // 遍历属性数组
        id value = [model valueForKey:key];
        if (value != nil) {                             // 判断字典中是否包含这个key
            [targetModel setValue:value forKeyPath:key];// 使用 KVC 设置数值
        }
    }
}
