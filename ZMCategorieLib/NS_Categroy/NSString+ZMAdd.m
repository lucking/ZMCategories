...
+ (NSString *)zm_removeStringEmpty:(NSString *)myStr {
    if (myStr!=nil) {
        ...
        str = [myStr stringByReplacingOccurrencesOfString:@"\n" withString:@""];
        return str;
    }
    return nil;
}
...
/* 替换字符串
 * range 替换的范围
 * replaceString 替换的后字符串
 */
- (NSString *)zm_ReplacinStringWithRange:(NSRange)range withString:(NSString *)replaceString {
    NSString *getSubString = [self substringWithRange:range];
    NSString *replaceResult = [self stringByReplacingOccurrencesOfString:getSubString withString:replaceString];
 ...
 *  @return 高度
 */
+ (CGFloat)zm_getTextSizeHeight:(NSString*)string Font:(UIFont*)myfont Width:(CGFloat)width
{
    //获取文本
    ...
    CGFloat height= mySize.height + 1;// ＋1 防止四舍五入
    return height;
}
/** 同上
 *  计算字符串的尺寸高度_height：宽度固定
                     ...
                     headIndent:(CGFloat)headIndent
{
    ...
    paraStyle.alignment = NSLineBreakByWordWrapping;
    ...
    //string
    CGSize mySize =[string boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attributeDic context:nil].size;
    return mySize.height + 1;// ＋1 防止四舍五入;
}
 ...
 *  @return 宽度
 */
+ (CGFloat)zm_getTextSizeWidth:(NSString*)string font:(UIFont *)myfont height:(CGFloat)height
{
    //获取文本
    NSMutableParagraphStyle *paragraph= [[NSMutableParagraphStyle alloc]init];
    paragraph.alignment = NSLineBreakByWordWrapping;
    ...
    //NSLog(@"-----> fontSize.height: %f \n ",mySize.height);
    return mySize.width + 1;// ＋1 防止四舍五入;
}
 ...
 *  不为空：@return YES
 */
+ (BOOL)zm_isNotEmptyString:(NSString *)string {
    
    if (!string) {                                                  return NO;
    }else if ([string isEqualToString:@""]) {                       return NO;
    }else if (string.length == 0) {                                 return NO;
    }else if (string == nil) {                                      return NO;
    }else if (string && [string isEqual:[NSNull null]]){            return NO;
    }else if ([string isKindOfClass:[NSNull class]]){               return NO;
    }else if ([string isEqualToString:@"<null>"]){                  return NO;
    }else if ([[string lowercaseString] isEqualToString:@"null"]){  return NO;
    }else{
        return YES;
    }
}
// 空的字符串nil/null 无法调用此方法, 就是空
- (BOOL)zm_isNotEmptyString {
    if (!self) {
        return NO;
    }
    else if ([self isEqualToString:@""]) {          return NO;
    }
    else if (self.length == 0) {                    return NO;
    }
    else if (self == nil) {                         return NO;
    }
    else if ([self isEqual:[NSNull null]]){         return NO;
    }
    else if ([self isKindOfClass:[NSNull class]]){  return NO;
    }
    else if ([self isEqualToString:@"<null>"]){     return NO;
    }
...
// 判断是否为空
- (BOOL)zm_isNull:(id)string
{
    if (!string) {                                                  return YES;
    }else if (string && [string isEqual:[NSNull null]]){            return YES;
    }else if ([string isKindOfClass:[NSNull class]]){               return YES;
    }else if (string == nil) {                                      return YES;
...
// 手机号 * 替换中间4位
+ (NSMutableString *)zm_phoneReplaceString:(NSString *)phoneStr
{
    NSMutableString *mutString = nil;
    if (phoneStr!=nil) {
        mutString= [[NSMutableString alloc] initWithString:phoneStr] ;
        if (phoneStr.length==11) { //11位卡号
            [mutString replaceCharactersInRange:NSMakeRange(3, 4) withString:@"****"];
            return mutString;
        }else{
            return mutString;
        }
