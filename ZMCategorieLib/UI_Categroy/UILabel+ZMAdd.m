...
- (void)zm_AddMiddleLine {
    ...
    [self setAttributedText:attribtStr];    
}
//中划线：指定文本
- (void)zm_AddMiddleLineText:(NSString *)text {
        
    NSMutableAttributedString *mutAttStr = [[NSMutableAttributedString alloc]initWithString:self.text];

    NSUInteger location = [[mutAttStr string] rangeOfString:text].location;
    NSUInteger length   = [[mutAttStr string] rangeOfString:text].length;
    NSRange _range = NSMakeRange(location, length);

//     [mutAttStr addAttribute:NSUnderlineStyleAttributeName value: [NSNumber numberWithInteger:NSUnderlineStyleSingle] range:_range]; 
    
    [mutAttStr addAttribute:NSStrikethroughStyleAttributeName value: [NSNumber numberWithInteger:NSUnderlineStyleSingle] range:_range];     
//    [mutAttStr addAttribute:NSUnderlineStyleAttributeName value:FFont(12) range:_range];
...
//下划线：指定文本
- (void)zm_AddUnderLineText:(NSString *)text {
...
+ (void)zm_setlineSpacingText:(UILabel *)label alignment:(NSTextAlignment)alignment lineSpacing:(CGFloat)lineSpacing {
    ...
    PghStyle.lineSpacing= lineSpacing;//调整行间距 4
